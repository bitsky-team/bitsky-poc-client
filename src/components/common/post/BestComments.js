...
export default class BestComments extends Component {
  ...
  getBestComments = async () => {
    const response = await axios.post(
      `${config.API_ROOT}/get_bestcomments`,
      qs.stringify({
  ...
  pushBestCommentsToState = comments => {
    let stateComments = []
  ...
  adjustMargin = () => {
    if (this.postComments) {
      let height = parseInt(
        Math.round(parseFloat(getComputedStyle(this.postComments).height))
      )
  ...
  toggle = () => {
    ...
    if (postComments && this.state.comments.length > 0) {
      let marginBottom = parseFloat(
        getComputedStyle(this.postComments).marginBottom.replace('px', '')
      )
      let translateY = this.getComputedTranslateY(this.postComments)
      let reversedHeight = parseInt(
        '-' +
          Math.round(
            parseFloat(
              getComputedStyle(this.postComments)
                .height.replace('px', '')
                .replace(',', '.')
            )
          )
      ...
      if (translateY === reversedHeight) {
        ...
        if (this.seeMoreButton) {
          this.seeMoreButton.style.display = 'flex'
        }
        let id = setInterval(() => {
          if (translateY === 0) {
            clearInterval(id)
            ...
            if (this.postComments) {
              this.postComments.style.transform =
    ...
    return (
      <div className="post-comments" ref={node => (this.postComments = node)}>
        ...
        <div
          ...
          ref={node => (this.seeMoreButton = node)}
        >
        ...
        <PostViewer
          ref={this.postViewer}
          id={this.props.id}
          toggleFavoriteFromActivityFeed={
            this.props.toggleFavoriteFromActivityFeed
          }
          increaseCommentCounterFromActivityFeed={
            this.props.increaseCommentCounterFromActivityFeed
          }
          decreaseCommentCounterFromActivityFeed={
